...
outdir <- "./5_pub.outs/3_example_plots/ALOFT_union/"
...
res2 <- res%>%filter(PIP>0.1, FDR<0.1)
...
enloc <- enloc%>%dplyr::select(gene=Gene, GLCP, PCG, LFDR, FDR_intact=FDR)
...
outdir2 <- "./5_pub.outs/3_example_plots/ALOFT_union/main/"
if (!file.exists(outdir2)) dir.create(outdir2, showWarnings=F, recursive=T)
...
## main
res3 <- res2%>%dplyr::filter(FDR_intact<0.1, peaking_d==3)
x <- res3%>%dplyr::select(symbol, genetic_variant, PIP, pval_gwas, chr, GLCP, PCG, FDR_intact)
...
figfn <- paste(outdir2, "Figure_", ens, "_", symbol, "_1.comb.png", sep="")       
...
resDAR <- read.xlsx("./5_pub.outs/3_example_plots/ALOFT_union/zzz_DAR_annot.xlsx")
...
###
  ...
  p0 <- CoveragePlot(atac3, region=region, show.bulk=F, peaks=F, annotation=F, tile=F, 
     group.by="treat", ymax=150, extend.upstream=0e+00, extend.downstream=0e+00, links=F,
...
comb <- plot_grid(pp1, pp2, pp_ls[[1]], pp_ls[[2]], pp_ls[[3]],
    align="hv", axis="lr", nrow=5, rel_heights=c(1.2, 1, 1, 1, 1))
  
figfn <- paste(outdir2, "Figure_", ens, "_", symbol, "_2.peak.png", sep="")       
png(figfn, width=800, height=1050, res=120)
print(comb)
dev.off()
...
##     align="hv", axis="lr", nrow=4, rel_heights=c(1.2, 1, 1, 1))
