...
###
### twas gene list
fn_ls <- sort(list.files ("./Asthma_twas/", pattern="ALOFT")) ## |Whole
...
opfn <- paste(outdir, gsub(".txt.gz", "", ii), ".xlsx", sep="")
openxlsx::write.xlsx(res, file=opfn, overwrite=T)
...
outdir2 <- "./4.3_example.outs/ALOFT_union/vcf/"
if (!file.exists(outdir2)) dir.create(outdir2, showWarnings=F)

###                            
### candidate genes
fn <-  "./4.3_example.outs/ALOFT_topPIP_twas.xlsx"
res <- openxlsx::read.xlsx(fn)

res2 <- res%>%filter(PIP>0.6, FDR<0.1)

###
mapDF <- str_split(res2$genetic_variant, ":", simplify=T)

res2 <- res2%>%mutate(chr=as.integer(mapDF[,1]), pos=as.integer(mapDF[,2]))
...
rm(list=ls()) 
## outdir2 <- "./4.3_example.outs/ALOFT_union/genes_list3/"
## if (!file.exists(outdir2)) dir.create(outdir2, showWarnings=F)

enloc <- read.table("./enloc_analysis/ALOFT_intact.txt", header=T)
enloc <- enloc%>%dplyr::select(gene=Gene, PCG, LFDR, FDR_intact=FDR)
...
bed2 <- bed%>%dplyr::select(id_b37=V4, chr_pos_grch38=V6)
...
res2 <- res%>%filter(FDR<0.1) ## FDR_intact<0.1)

## res2 <- res%>%filter(PIP>0.7, FDR<0.1)


###
mapDF <- str_split(res2$genetic_variant, ":", simplify=T)
res2$chr_pos_grch37 <- paste(mapDF[,1], mapDF[,2], sep="_")
res2 <- res2%>%left_join(bed2, by=c("genetic_variant"="id_b37"))
...
## fn <- "./4.3_example.outs/ALOFT_topPIP_twas.xlsx"
## tmp <- openxlsx::read.xlsx(fn)

## tmp2 <- tmp%>%filter(PIP>0.8, FDR<0.1)
## ## add annotation
## tmp2 <- tmp2%>%left_join(anno_torus, by=c("genetic_variant"="SNP"))
...
###
### gwas 
fn <- "./gwas_asthma/imputed_UKB_20002_1111_self_reported_asthma.txt.gz"
gwas <- fread(fn, header=T, data.table=F)%>%
    mutate(chr_pos_grch38=gsub("chr", "", paste(chromosome, position, sep="_")),
           chr=gsub("chr", "", chromosome))


###
### dap results 
fn <- "./ALOFT_results/dap_allSNP.txt.gz"
...
###
### FastQTL results
fn <- "./ALOFT_results/PC1-18.nominals.eQTL.txt.gz"
...
###
### signac object
fn <- "../analyses.2021-02-05/1_processing/5.1_reCallPeak.outs/3_scATAC.annot.rds"
atac <- read_rds(fn)
...
###
### sample list
sampleID <- read.table("./4.3_example.outs/sampleID.txt")$V1
...
dd <- 3
outdir2 <- "./4.3_example.outs/ALOFT_union/genes_list_intact_topPIP0.5_dd3/"
if (!file.exists(outdir2)) dir.create(outdir2, showWarnings=F)
 
res3 <- res2%>%filter(peaking_d==dd, PIP>0.5, FDR_intact<0.1)
...
###
### vcf file
genfn <- paste("./4.3_example.outs/ALOFT_union/vcf/", ens, "_", symbol, "_", chr_i, "_dosages.txt", sep="")
...
bed3 <- bed%>%dplyr::select(chr_pos_grch37=V3, chr_pos_grch38=V6)
gwas2 <- gwas%>%filter(chr==chr_i)%>%
    left_join(bed3, by="chr_pos_grch38") ##%>%drop_na(chr_pos_grch37)

gwas2 <- gwas2%>%
   mutate(pos=as.numeric(gsub(".*_", "", chr_pos_grch37)),
          pos2=pos/1e+03, log10p=-log10(pvalue))
 
gwas3 <- gwas2%>%
    filter(pos>(pos_i-dtss1), pos<(pos_i+dtss2))%>%
    left_join(ldDF2, by="chr_pos_grch37")

gwas3 <- gwas3%>%mutate(gr2=ifelse(is.na(r2), 6, gr))%>%mutate(gr2=ifelse(r2==1, 0, gr2))
...
p1 <- ggplot(gwas3, aes(x=pos2, y=log10p))+
   ...
   geom_text_repel(data=anno, aes(x=pos2, y=log10p, label=variant_id),
...
fast3 <- fast3%>%mutate(gr2=ifelse(is.na(r2), 6, gr))%>%mutate(gr2=ifelse(r2==1, 0, gr2))    
...
###
comb <- plot_grid(p1, p2, p3, p4, align="hv", axis="lr", nrow=4, rel_heights=c(0.7, 0.6, 0.6, 0.5))
...
outdir2 <- "./4.3_example.outs/ALOFT_union/genes_list_intact_dd3/"
if (!file.exists(outdir2)) dir.create(outdir2, showWarnings=F)
...
pos_max <- as.integer(pos_i+2000)

symbol2 <- symbol
##condition <- DFanno%>%filter(symbol==symbol2)%>%pull(conditions)%>%unique()
oneMCl <- "NKcell" #gsub("_.*", "", condition)
ii <- "PHA-DEX"  ## gsub(".*_", "", condition)
i1 <- contrast_ls[[ii]][1]
i2 <- contrast_ls[[ii]][2]
...
peak_i <- resDAR%>%filter(symbol==symbol2)%>%pull(gene)%>%unique()
...
pp2 <- CoveragePlot(atac2, region=region, show.bulk=F, peaks=F, annotation=F, tile=F, 
   ...
   annotate("point",x=pos_i, y=0.5, shape=18, size=3, color="blue")&
...
###
  ...
  p0 <- CoveragePlot(atac3, region=region, show.bulk=F, peaks=F, annotation=F, tile=F, 
     group.by="treat", ymax=100, extend.upstream=0e+00, extend.downstream=0e+00, links=F,
...
comb <- plot_grid(pp1, pp2, pp_ls[[1]], pp_ls[[2]], pp_ls[[3]],
    align="hv", axis="lr", nrow=5, rel_heights=c(1.3, 1, 1, 1, 1))
  
figfn <- paste(outdir2, "Figure", i, ".2_", ens, "_", symbol, "_peak.png", sep="")       
png(figfn, width=800, height=1050, res=120)
print(comb)
dev.off()
...
### 2-genotype   
## comb <- plot_grid(pp1, pp2, pp_ls[[2]], pp_ls[[3]],
##     align="hv", axis="lr", nrow=4, rel_heights=c(1.3, 1, 1, 1))
  
## figfn <- paste(outdir2, "Figure", i, ".2_", ens, "_", symbol, "_peak.png", sep="")       
## png(figfn, width=800, height=950, res=120)
## print(comb)
## dev.off()
...
opfn <- "./4.3_example.outs/ALOFT_union/zzz_motif_annot.xlsx"
...
opfn <- "./4.3_example.outs/ALOFT_union/zzz_DAR_annot.xlsx"
...
###
opfn <- paste("4.3_example.outs/ALOFT_union/tmp_", symbol0, "_motif.xlsx", sep="")
...
tmp2 <- resDAR%>%filter(symbol==symbol0)%>%arrange(desc(abs(estimate)))%>%
    mutate(conditions=paste(MCls, contrast, sep="_"),
           LFC=round(estimate, 3), stderror=round(stderror, 3), pval=round(p.value, 3))%>%
    filter(MCls!="DC")%>%
    dplyr::select(conditions, gene, LFC, stderror, pval)    

####
opfn <- paste("4.3_example.outs/ALOFT_union/tmp_", symbol0, "_DAR.xlsx", sep="")
