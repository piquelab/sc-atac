...
pct0 <- 0.01    
option <- paste("torus_peak_pct_", pct0, sep="") ## "_response3", sep="")
...
###
###"
p <- ggplot(est2, aes(x=V2, y=as.factor(treats),color=factor(treats)))+
      geom_errorbarh(aes(xmax=V4, xmin=V3), size=0.5, height=0.2)+
      geom_point(shape=19, size=0.5)+
      geom_vline(aes(xintercept=0), size=0.25, linetype="dashed")+
      xlab("log odds ratio")+
...
###
figfn <- paste(outdir, "Figure0.1_Tcell_peak.est.png", sep="")
png(figfn, width=350, height=480, res=120)
print(p)
dev.off()
