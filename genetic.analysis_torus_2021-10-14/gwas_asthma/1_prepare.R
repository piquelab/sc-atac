...
## ###
## summ2 <- fread("./gwas_input/imputed_UKB_20002_1111_self_reported_asthma.txt.gz", data.table=F)
## summ2 <- summ2%>%mutate(chr_pos=paste(gsub("chr", "", chromosome), position, sep="_"))%>%
##     dplyr::select(SNP=variant_id, chr_pos)

## chr_pos <- summ2$chr_pos
## names(chr_pos) <- summ2$SNP

## ##
## summ$chr_pos <- chr_pos[as.character(summ$SNP)]
## gfn <- gzfile("./gwas_input/Asthma_torus_zval.txt.gz")
## write.table(summ, gfn, row.names=F, col.names=T, quote=F, sep="\t")
...
###
gfn <- gzfile("./torus_input/Asthma_torus_zval.txt.gz")
