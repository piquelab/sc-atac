...
summ <- map_dfr(1:length(motifList), function(i){
   ...
   cat(i, motif, "\n")    
   ###
   fn <- paste(outdir, motif, "_union_torus.annot.gz", sep="")
   if ( file.exists(fn)){
      ### 
      annot <- fread(fn, header=T, data.table=F)
      summ2 <- data.table(motif=motif, MCls=oneMCl, num=sum(annot$peaking_d==2)) 
      summ2
   }    
})

opfn <- paste("./torus_input/Summary_", oneMCl, "_", motifFile, ".txt", sep="")
