...
### (2). merge files
bcftools merge `find ./TMP/ -name 'SCAIP6*.bcf'` --threads 5  -Oz -o SCAIP6.merge.vcf.gz 
...
#subset vcf
bcftools view -T <(zcat SCAIP6.posG100.txt.gz) /wsu/home/groups/piquelab/SCAIP/vcf/SCAIP1-6.vcf.gz --threads 5 -Oz -o SCAIP6.posG100.AL.vcf.gz
...
bcftools view SCAIP6.posG100.AL.vcf.gz -S ../../sample6.id --threads 5 -Oz -o sample6.posG100.AL.vcf.gz
## MAF
bcftools plugin fill-tags sample6.posG100.AL.vcf.gz -- -t 'AN,AC,AF,MAF'|bcftools view --threads 5 -Oz -o sample6.posG100.AF.vcf.gz 
## filter by MAF
bcftools view sample6.posG100.AF.vcf.gz -i 'INFO/MAF>0.05' --threads 5 -Oz -o sample6.posG100.MAF005.vcf.gz
